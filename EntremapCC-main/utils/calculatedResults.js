function calculateResults(SurveyResults) {
  let resultSet = {
    ...
    RuleBreaking: { Total: 0, Average: 0 },
    Curiosity: {
      JoyousExploration: 0,
      DeprivationSensitivity: 0,
      StressTolerance: 0,
      ThrillSeeking: 0,
      OvertSocial: 0,
      CovertSocial: 0,
  ...
  for (let [key, value] of Object.entries(SurveyResults)) {
    ...
    if (key.includes("MAWINTRI")) {
      ...
      resultSet.RuleBreaking.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("CUJE")) {
      resultSet.Curiosity.JoyousExploration += parseInt(SurveyResults[key]);
      resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("CUDS")) {
    resultSet.Curiosity.DeprivationSensitivity += parseInt(SurveyResults[key]);
    resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("CUST")) {
    resultSet.Curiosity.StressTolerance += parseInt(SurveyResults[key]);
    resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("CUTS")) {
      resultSet.Curiosity.ThrillSeeking += parseInt(SurveyResults[key]);
      resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("SOCUO")) {
      resultSet.Curiosity.OvertSocial += parseInt(SurveyResults[key]);
      resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    } else if (key.includes("SOCUC")) {
      resultSet.Curiosity.CovertSocial += parseInt(SurveyResults[key]);
      resultSet.Curiosity.Total += parseInt(SurveyResults[key]);
    }
